...
def test_group_robot():
    with open('robot_access_token', 'r') as fp:
...
def test_app():
    with open('corp_id', 'r') as fp:
        corp_id = fp.read().strip()

    with open('corp_secret', 'r') as fp:
    ...
    status = app.send_text(access_token=access_token, userid_list=userid_list, text='Hello')
